...
from ultralytics.nn.modules import (C1, C2, C3, C3TR, SPP, SPPF, Bottleneck, BottleneckCSP, C2f, C3Ghost, C3x, Classify,
                                    ...
                                    GhostBottleneck, GhostConv, Pose, Segment, DSPPF, WeightedFusion, C2f_EMA)
...
class BaseModel(nn.Module):
    ...
    def _forward_once(self, x, profile=False, visualize=False):
        ...
        for m in self.model:
            ...
            x = m(x)  # run
            #print(f"Layer {m} ({type(m).__name__}): Output shape = {x.shape if isinstance(x, torch.Tensor) else [xi.shape for xi in x]}")
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):
        ...
        # WeightedFusion must come first
        if m is WeightedFusion:
            assert isinstance(args, dict), f"Expected dict for WeightedFusion args, got {args}"
            input_channels = [ch[x] for x in f]
            c2 = args['c_out']
            args = [input_channels, c2]  # correct format for your class

        # Then regular modules
        elif m in (Classify, Conv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, Focus,
                  BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, nn.ConvTranspose2d, DWConvTranspose2d, C3x, DSPPF, C2f_EMA):
            ...
            args = [c1, c2, *args[1:]]
            if m in (BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, C3x, C2f_EMA):
                args.insert(2, n)
                n = 1
