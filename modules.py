...
class DSPPF(nn.Module):
    ...
    def forward(self, x):
        ...
        if torch.isnan(out).any():
            print("[DEBUG:DSPPF] cv2 output contains NaNs.")
...
class WeightedFusion(nn.Module):
    def __init__(self, c_list, c_out):
        super().__init__()
        assert len(c_list) == 3, "Expected exactly 3 input feature maps"

        self.n = 3  # number of inputs
        self.w = nn.Parameter(torch.ones(self.n, dtype=torch.float32))  # learnable weights

        # 1x1 projection convs to unify channel dims
        self.proj = nn.ModuleList([
            nn.Conv2d(c, c_out, kernel_size=1, stride=1, padding=0, bias=False)
            for c in c_list
        ])
    ...
    def forward(self, x):
        assert isinstance(x, (list, tuple)) and len(x) == self.n, f"Expected list of {self.n} tensors"

        for i, xi in enumerate(x):
            if torch.isnan(xi).any():
                print(f"[DEBUG] Input x[{i}] contains NaNs.")
            if torch.isinf(xi).any():
                print(f"[DEBUG] Input x[{i}] contains Infs.")

        # Project all inputs to same channel count
        projected = [self.proj[i](x[i]) for i in range(self.n)]

        for i, pi in enumerate(projected):
            if torch.isnan(pi).any():
                print(f"[DEBUG] proj[{i}] output contains NaNs.")
            if torch.isinf(pi).any():
                print(f"[DEBUG] proj[{i}] output contains Infs.")

        # Use absolute weights and normalize manually (not softmax)
        weights = torch.abs(self.w) + 1e-4  # ensure positive
        weights = weights / weights.sum()

        fused = sum(weights[i] * projected[i] for i in range(self.n))

        if torch.isnan(fused).any():
            print("[DEBUG] Output after weighted sum contains NaNs.")
        if torch.isinf(fused).any():
            print("[DEBUG] Output after weighted sum contains Infs.")

        fused = self.bn(fused)
        if torch.isnan(fused).any():
            print("[DEBUG] Output after BatchNorm contains NaNs.")
        fused = self.act(fused)
        if torch.isnan(fused).any():
            print("[DEBUG] Output after activation contains NaNs.")
        return fused


#print(f"[WeightedFusion] weights: {w.detach().cpu().numpy()}")

# Assuming Conv and Bottleneck are already implemented in your module.py
...
class Conv(nn.Module):
    ...
    def forward(self, x):
        #if torch.isnan(x).any():
        #    print(f"[Conv] WARNING: NaN detected in input! Shape: {x.shape}")
        return self.act(self.bn(self.conv(x)))

    def forward_fuse(self, x):
        #if torch.isnan(x).any():
        #    print(f"[Conv-Fused] WARNING: NaN detected in input! Shape: {x.shape}")
        return self.act(self.conv(x))
